...
it('lists branches by user', function () {
    ...
    $branchService->shouldReceive('filterByUser')
        ->with('testworkspace', 'testrepo', 'Jane')
        ->andReturn([$fixture['values'][1]]);
    $this->app->instance(BranchService::class, $branchService);

    $this->artisan('branch:user Jane --project=testworkspace/testrepo')
