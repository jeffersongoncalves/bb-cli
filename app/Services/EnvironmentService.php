...
class EnvironmentService
{
    ...
    {
        return $this->bitbucket->getRepo($workspace, $repoSlug, 'environments/');
    }

    public function variables(string $workspace, string $repoSlug, string $environmentUuid): array
    {
        return $this->bitbucket->paginateRepo($workspace, $repoSlug, "deployments_config/environments/{$environmentUuid}/variables");
    }
    ...
    {
        return $this->bitbucket->postRepo($workspace, $repoSlug, "deployments_config/environments/{$environmentUuid}/variables", [
            'key' => $key,
            'value' => $value,
            'secured' => $secured,
        ]);
    ...
    {
        return $this->bitbucket->putRepo($workspace, $repoSlug, "deployments_config/environments/{$environmentUuid}/variables/{$variableUuid}", [
            'value' => $value,
            'secured' => $secured,
        ]);
    }
